...
def add_bcs_to_weak_form(u, W_open, z, g, wd_alpha_sq, domain, ds, F_u, F_u_open, p, F, wd=True):
    ...
    # Compute jump conditions for wall boundaries
    jump_Q_wall = as_vector((0, 2 * H * un * n[0], 2 * H * un * n[1]))
    C_wall = vnorm + sqrt(g * H)

    # Compute reflection condition for solid wall boundary
    u_wall = as_vector((
        u[0],  
        u[1] * n[1] * n[1] - u[1] * n[0] * n[0] - 2 * u[2] * n[0] * n[1], 
        u[2] * n[0] * n[0] - u[2] * n[1] * n[1] - 2 * u[1] * n[0] * n[1]
    ))

    # Compute flux tensor at the wall boundary
    Fu_wall_ext = make_Fu(u_wall, z, wd_alpha_sq, g, wd=True)

    # Compute jump conditions for open boundary
...
def dg_flux(u, z, g, wd_alpha_sq, domain, F_u, p, F, Q, wd=True):
    ...
    Returns:
    Updated weak form equation F with DG flux applied.
    """
    eps = 1e-16  # Small value to avoid division by zero
    
    # Compute unit normal to the element interface
    n = ufl.FacetNormal(domain)

    # Get standardized solution variables
    H, ux, uy = get_standard_vars(u, z, wd_alpha_sq, form='H', wd=True)

    # Compute velocity vectors on both sides of the element interface
    vela = as_vector((ux('+'), uy('+')))  # Velocity on side '+'
    velb = as_vector((ux('-'), uy('-')))  # Velocity on side '-'

    # Compute velocity norms, ensuring no division by zero
    vnorma = conditional(dot(vela, vela) > eps, sqrt(dot(vela, vela)), 0.0)
    vnormb = conditional(dot(velb, velb) > eps, sqrt(dot(velb, velb)), 0.0)

    # Compute the numerical wave speed using Roe-like upwinding
    C = conditional(
    ...
    # Compute the flux term using the average flux and a jump stabilization term
