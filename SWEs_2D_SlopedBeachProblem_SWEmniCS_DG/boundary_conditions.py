...
def initialize_boundary_conditions(V, W_open, domain, Ly=7200.0):
    """
    ...
    # Define the open boundary: x = 0
    def open_boundary(x):
        return np.isclose(x[0], 0)

    # Define the wall boundary: all other boundaries except x = 0
    def wall_boundary(x):
    ...
    sub_dofs_h_open_left = fem.locate_dofs_geometrical(
        (V_boundary.sub(0), V_boundary.sub(0).collapse()[0]), lambda x: np.isclose(x[0], 0)
    )[0]
