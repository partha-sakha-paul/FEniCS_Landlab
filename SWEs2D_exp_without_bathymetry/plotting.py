...
def plot_station_timeseries(time_series_data, station_coords):
    ...
    for coord in station_coords:
        ...
        plt.subplot(3, 1, 1)
        plt.plot(time_series_data[coord]["time"], time_series_data[coord]["h"], label="Water Depth Elevation (h)")
        ...
        plt.tight_layout()
        plt.savefig(f"station_{coord}_timeseries_exp_without_bathymetry_CG.png")  # Save plot as image
        plt.show()
