...
# Extracting the coords of the mesh
x = domain.geometry.x

# initializing current functions with zeros as dry bed
h.interpolate(lambda x: np.full_like(x[0], 0.6))
...
h_prev.interpolate(lambda x: np.full_like(x[0], 0.6))
...
def weak_form(U, U_prev, V_test, wd = True):
    ...
    # Continuity equation
    continuity = (h - h_prev) / dt * v_h * dx + ((h * ux).dx(0) * v_h + (h * uy).dx(1) * v_h) * dx
    # Momentum equations
    momentum_x = (h * (ux - ux_prev) ) / dt * v_ux * dx + g * h * h.dx(0) * v_ux * dx + g * mannings**2 * ((abs(ux) * ux) / h_corrected**(1/3)) * v_ux * dx
    momentum_y = (h * (uy - uy_prev) ) / dt * v_uy * dx + g * h * h.dx(1) * v_uy * dx + g * mannings**2 * ((abs(uy) * uy) / h_corrected**(1/3)) * v_uy * dx
...
for step in range(num_steps + 1):
    ...
    if step in desired_steps:
        ...
        plotter.add_mesh(
            warped,
            scalars="Water Depth (h)",  # Use water surface elevation as scalar
            cmap="viridis",
            show_edges=True,
